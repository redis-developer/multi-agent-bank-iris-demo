...
- **VERIFY BEFORE THE DAY — Section 4 against a live service**: the
  ...
  (console **Create with CLI** → admin key, deploy, generated tool
  names, MCP calls — the client integration is built against
  ...
  github.com/redis/redis-iris-demos). Admin keys can only be minted
  with a console session — the console step can't be scripted away.
