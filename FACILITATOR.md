...
5. **Free local ports**: 80 (workbench), 3000, 3001, 8000, 5540.
...
- **No Redis Cloud accounts on the day**: Sections 4 (Context Retriever),
  5 (Agent Memory), and 6 (LangCache) all provision required cloud
  services. Put "create a free Redis Cloud account + free database"
...
- **Section 6 LangCache LIVE-VERIFIED (2026-08-14)** against a real
  ...
  Section 5's app integration with the *managed* Agent Memory service —
  the payloads follow the published API reference, but rehearse the
  full amnesia → recall flow against a real service before the day.
...
- **"My solution doesn't work"**: the api auto-reloads on save (uvicorn
  ...
  reloads when it's saved. One footgun:
