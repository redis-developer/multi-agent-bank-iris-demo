...
2. **Save and test — the question RAG can't answer.** As **Ananya Sharma
   ...
   The reply is generic — and it has to be. Everything the bot can
   retrieve so far is the FAQ knowledge base: *documents*. RAG can
   explain what "outstanding principal" means; it cannot know **your**
   outstanding, because that isn't written in any document — it's a row
   in the loan table, filtered to you. Documents hold shared knowledge;
   answering this question takes **entities**.
   ...
   tools are hand-written, but every READ tool comes from
   `context_read_tools()`, a surface generated by the Context Retriever,
   and nothing is deployed yet. See for yourself in the **Terminal**
   panel:
